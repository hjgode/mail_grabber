...
namespace ews_grabber
{
    class ews:IDisposable
    {
        ...
        public delegate void onStateChangedDelegate(StatusEventArgs args);
        public event onStateChangedDelegate stateChanged;
        protected void onStateChanged(StatusEventArgs args)
        {
            System.Diagnostics.Debug.WriteLine("onStateChanged: " + args.eStatus.ToString() + ":" + args.message);
            onStateChangedDelegate handler = stateChanged;
            if (handler != null)
            {
                handler(args);
            }
                ...
                do
                {
                    ...
                    foreach (Item item in findResults.Items)
                    {
                        ...
                        if (item is EmailMessage)
                        {
                            ...
                            _ews.onStateChanged(new StatusEventArgs(StatusType.license_mail, (item as EmailMessage).Sender.Name + ": " + (item as EmailMessage).Subject));
                        }
                    }
                    ...
                    foreach (ItemEvent itemEvent in events.ItemEvents)
                    {
                        switch (itemEvent.EventType)
                        {
                            case EventType.NewMail:
                                ...
                                _ews.onStateChanged(new StatusEventArgs(StatusType.license_mail, message.Sender + message.Subject));
                                break;
