...
namespace ews_grabber
{
    class ews:IDisposable
    {
        ...
        string hsm_service_url = "https://az18-cas-01.global.ds.honeywell.com/EWS/Exchange.asmx";
        ...
        public List<EmailMessage> _mailList=new List<EmailMessage>();
        ...
        public ews()
        {
            try
            {
                ...
                //service = new Microsoft.Exchange.WebServices.Data.ExchangeService();// (Microsoft.Exchange.WebServices.Data.ExchangeVersion.Exchange2010_SP2);                
        ...
        public class ReadMailEventArgs : EventArgs
        {
            public bool bValid;
            public ReadMailEventArgs(bool b)
            {
                bValid = b;
            }
        }
        public delegate void onMailReadDelegate(ReadMailEventArgs args);
        public event onMailReadDelegate MailRead;
        protected void onMailRead(ReadMailEventArgs args)
        {
            if (MailRead != null)
            {
                MailRead(args);
        ...
        public void _getMails(object param)
        {
            ...
            try
            {
                ...
                */
                ...
                do
                {
                    ...
                    findResults = service.FindItems(
                        ...
                        new SearchFilter.SearchFilterCollection(
                            LogicalOperator.And,
                            new SearchFilter.ContainsSubstring(ItemSchema.Subject, "License Keys - Order"),
                            new SearchFilter.ContainsSubstring(ItemSchema.Attachments, "xml")),
                            view);
                    foreach (Item item in findResults.Items)
                    {
                        ...
                        if (item is EmailMessage)
                        {
                            ...
                            _mailList.Add(item as EmailMessage);
                ...
                } while (findResults.MoreAvailable && _bRunThread);
                onMailRead(new ReadMailEventArgs(true));
            }
            ...
            {
                helpers.addLog("Exception: " + ex.Message);
                onMailRead(new ReadMailEventArgs(false));
            }
        ...
        void startPullNotification(){
            ...
            PullSubscription subscription = service.SubscribeToPullNotifications( new FolderId[] { WellKnownFolderName.Inbox },
            ...
            foreach (FolderEvent folderEvent in events.FolderEvents)
            {
                switch (folderEvent.EventType)
                {
                    case EventType.Created:
                        ...
                        Folder folder = Folder.Bind(service, folderEvent.FolderId);
                        break;
                    case EventType.Deleted:
